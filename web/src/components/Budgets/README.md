...
- **Currency weights instead of conversion matrix**: Multi-currency conversion uses a flat `Record<string, number>` of weights (value per unit relative to main currency) rather than an NxN matrix. Converting A→B is `amount * weights[A] / weights[B]`, which is simpler and avoids O(n²) pre-computation.
