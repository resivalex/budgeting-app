...
- **Budget Name**: Link the transaction to a named budget. Auto-filled when a category with a matching budget is selected; budgets matching the current category appear first. On mobile, searchable fullscreen overlay with a floating confirm button to keep the current value.
