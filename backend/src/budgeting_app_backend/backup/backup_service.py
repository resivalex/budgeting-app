...
class BackupService:
    """Creates and restores full backups of CouchDB databases."""
    ...
    def restore_from_zip(self, zip_bytes: bytes) -> Dict[str, Any]:
        ...
        with zipfile.ZipFile(buf, "r") as zf:
            ...
            if SETTINGS_ARCHIVE_PATH not in archive_names:
                raise ValueError(f"Missing {SETTINGS_ARCHIVE_PATH} in archive")

            couchdb_bytes = zf.read(COUCHDB_ARCHIVE_PATH)
            settings_bytes = zf.read(SETTINGS_ARCHIVE_PATH)
    ...
    def _dump_settings_as_couchdb(self) -> bytes:
        server = pycouchdb.Server(self._db_url)
        if SETTINGS_DB_NAME not in server:
            server.create(SETTINGS_DB_NAME)
        db = server.database(SETTINGS_DB_NAME)
