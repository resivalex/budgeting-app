...
- **`GET /exporting`**: Returns all transactions as a CSV file with account IDs resolved to human-readable names
...
- **Upload Timestamp**: Tracks the last server-side database change; frontend compares this to detect a database reset and trigger a full resync

### Data Export

- **CSV Export**: Exports all CouchDB transactions as CSV with columns: `datetime, account, category, type, amount, currency, payee, comment, budget_name`, sorted newest first. Account IDs are resolved to human-readable names using `cfg:account_properties`; for transfer transactions, the payee (destination account ID) is also resolved
- **`budget_name` field**: Present in export CSV; missing values become empty string (`""`)
